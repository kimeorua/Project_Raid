...
void UPR_UIComponent_Player::InitComponent()
{
	...
	if (ASC && HUD_Player)
	{
		...
		HUD_Player->UpdateSPBar_Owner(CurrentStamina / CurrentMaxStamina);
	}
	TryInitializeNetworkMultiplayerUI();
}

void UPR_UIComponent_Player::CreateCombatUI(TSubclassOf<UPR_WeaponCombatUI> InWeaponCombatUI)
{
	APlayerController* LocalPC = Cast<APlayerController>(OwnerCharacter->GetController());
	if (HUD_Player)
	{
		...
		if (LocalPC && LocalPC->IsLocalController())
		{
			if (!InWeaponCombatUI) { return; }

			UUserWidget* WeaponCombatUI = CreateWidget<UUserWidget>(LocalPC, InWeaponCombatUI);
			if (!WeaponCombatUI) { return; }

			HUD_Player->AddCombatUI(Cast<UPR_WeaponCombatUI>(WeaponCombatUI));
		}
	}
}
...
void UPR_UIComponent_Player::OnSPChanged(const FOnAttributeChangeData& Data)
{
	if (HUD_Player && ASC)
	{
		float MaxSP = ASC->GetNumericAttribute(UPR_BasicAttributeSet::GetMaxSPAttribute());
        
		HUD_Player->UpdateSPBar_Owner(Data.NewValue / MaxSP);
	}
}

void UPR_UIComponent_Player::OnMaxSPChanged(const FOnAttributeChangeData& Data)
{
	if (HUD_Player && ASC)
	{
		float CurrentSP = ASC->GetNumericAttribute(UPR_BasicAttributeSet::GetSPAttribute());

		HUD_Player->UpdateSPBar_Owner(CurrentSP / Data.NewValue);
	}
}
